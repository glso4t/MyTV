...
public class RegisterFrame extends JFrame {
	...
	private JTextField firstNameField, lastNameField, usernameField, passwordField, confirmPasswordField;
					...
					if(!usernameAlreadyExists && !emailAlreadyExists) {
						if(password.equals(confirmPassword)) {
							if(password.length() >= 8) {
								UserAccount acc = new UserAccount(firstName, lastName, username, password);
								db.addUserAccount(acc);
								
								JOptionPane.showMessageDialog(null, "User has been created and stored to the system !!");
								dispose();
							}
